...
public class 가로막대차트 extends JFrame {
	...
	private void getData(int sno) {
		try (var rs = Connect.res("select name, dayofweek(t.date) -1 day, sum(t.adult) adult, sum(t.youth) youth, sum(t.kid) kid from ticket t join location using(lno) join science using(sno) where sno = "+sno+" group by dayofweek(t.date) order by day;")) {
